...
int pthread_create(pthread_t *threadId, void* notUsed, void *(*__start_routine) (void *), void* userParm) {
	...
	//HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, __start_routine, userParm, 0, &dwThreadId);
	...
	if (*threadId != NULL)
		...
		return(GetLastError()); //SetLastError(SE_WORKER_THREAD_CREATE, __FUNCTION__, ::GetLastError());
}
...
void pthread_exit(void *retval) {
	if(retval == NULL)
		...
		ExitThread((unsigned long *)retval);	//_endthreadex((unsigned int *)retval);
}
#endif //REDIS_PTHREAD
...
int pthread_mutex_init(pthread_mutex_t *mutex, const pthread_mutexattr_t *attr) {
	if (0 == InitializeCriticalSectionAndSpinCount(mutex, 0))
		return(0);
	else
		return(1);
}
