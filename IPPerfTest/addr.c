...
int ip_addr_cmp(struct host_addr *a, struct host_addr *b)
{
	if (a->family == AF_INET && b->family == AF_INET) {
		if (a->address.ipv4.s_addr == b->address.ipv4.s_addr) return 0;
		else if (a->address.ipv4.s_addr > b->address.ipv4.s_addr) return 1;
		else return -1;
	}
#if defined ENABLE_IPV6
	if (a->family == AF_INET6 && b->family == AF_INET6) {
		return(ip6_addr_cmp(&a->address.ipv6, &b->address.ipv6));
	}
	else if (a->family == AF_INET && b->family == AF_INET6) {
		return 1;
	}
...
/*
 * sa_addrport_cmp(): compare two IP addresses with Portnr: both encapsulated into a
 * 'struct sockaddr'.
 * returns 0 if they match; 1 if they don't match; -1 to signal a generic
 * error (e.g. unsupported family mismatch).
 */
int sa_addrport_cmp(struct sockaddr *sa, struct sockaddr *a)
{
	struct sockaddr_in *sa4 = (struct sockaddr_in *)sa, *da4 = (struct sockaddr_in *)a;
#if defined ENABLE_IPV6
	struct sockaddr_in6 *sa6 = (struct sockaddr_in6 *)sa;
	struct sockaddr_in6 sa6_local;
	struct sockaddr_in6 *da6 = (struct sockaddr_in6 *)a;
	struct sockaddr_in6 da6_local;
#endif

	if (a->sa_family == AF_INET && sa->sa_family == AF_INET) {
		if (sa4->sin_addr.s_addr == da4->sin_addr.s_addr) {
			if (sa4->sin_port == da4->sin_port)
				return FALSE;
			else
				return TRUE;
		}
...
#if defined ENABLE_IPV6
	//ToDo:
	//	IPv6 Compare is wrong must be rewriten !!!
	if (a->sa_family == AF_INET6 && sa->sa_family == AF_INET6) {
		if (!ip6_addr_cmp(&sa6->sin6_addr, &da->sin6_addr)) return FALSE;
		else return TRUE;
	}
	else if (da->sa_family == AF_INET && sa->sa_family == AF_INET6) {
		memset(&sa6_local, 0, sizeof(sa6_local));
		memset((u_int8_t *)&sa6_local.sin6_addr + 10, 0xff, 2);
		memcpy((u_int8_t *)&sa6_local.sin6_addr + 12, &da->sin6_addr, 4);
		if (!ip6_addr_cmp(&sa6->sin6_addr, &sa6_local.sin6_addr)) return FALSE;
		else return TRUE;
	}
	else if (a->sa_family == AF_INET6 && sa->sa_family == AF_INET) {
		memset(&sa6_local, 0, sizeof(sa6_local));
		memset((u_int8_t *)&sa6_local.sin6_addr + 10, 0xff, 2);
		memcpy((u_int8_t *)&sa6_local.sin6_addr + 12, &sa4->sin_addr, 4);
		if (!ip6_addr_cmp(&sa6_local.sin6_addr, &da->sin6_addr)) return FALSE;
		else return TRUE;
	}
#endif
